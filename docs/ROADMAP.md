...
- Chat can now create Strategic Goal confirmation cards from explicit goal-setting language; confirmed goals use the existing Goal service, Audit, persistence, and restart recovery path, ordinary conversation includes active goal context, "next step" / `下一步` messages continue the active goal through a governed Agent Run or local controlled task plan, and new Workflow/Agent Run step tasks auto-link to the current active goal.
