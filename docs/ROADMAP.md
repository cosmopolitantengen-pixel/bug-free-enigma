...
The Next.js console satisfies the production UI migration, while the static shell remains as a fallback. The console can store an operator bearer token for protected API calls. Automated browser-level E2E CI, release automation, managed alert routing/escalation, managed identity, and full production session hardening remain.
