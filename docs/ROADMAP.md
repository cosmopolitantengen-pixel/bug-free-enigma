...
- Durable one-time and recurring internal schedules can create or run tasks through an explicit Human Root tick, with lifecycle controls, execution history, Redis/RQ queue health, incidents, optional outbound alerts, audit events, and append-only domain events.
...
The Next.js console satisfies the production UI migration, while the static shell remains as a fallback. The console can store an operator bearer token for protected API calls. Automated browser-level E2E CI, managed alert routing/runbooks, managed identity, and full production session hardening remain.
