...
The Next.js chat workspace sends ordinary conversation turns through this same generation boundary. It sends a bounded recent transcript, keeps conversation text in browser-local storage, and renders routing/usage/cost metadata from the response. Explicit operational language can instead produce an audited Workflow proposal. Known requests use deterministic rules and spend no model tokens. When an auto/action request looks operational but rules cannot classify it, a selected non-local provider may run the `chat_action_planning` purpose through the same Budget, usage, CostLog, fallback, and Audit controls. The parser accepts only a small intent catalog plus optional search text or an allowlisted collaboration target; executable inputs remain fixed server mappings. Planner usage and cost are returned with the proposal. No proposal creates a task until Human Root confirms it. Provider credentials remain server-side; the browser never receives or stores `DEEPSEEK_API_KEY` or `OPENAI_API_KEY`.
