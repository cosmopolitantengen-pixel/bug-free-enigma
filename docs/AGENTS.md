...
All 18 V1 roles are registered at bootstrap with scoped permissions, forbidden actions, Skills, Tools, reporting lines, and risk levels. The dedicated Workspace Agent owns approval-gated patch and command capabilities without silently expanding persisted Tech Agent permissions. Catalog validation rejects unknown Skill, Tool, or manager references, rejects asymmetric Agent/Skill authorization, and continues to prohibit `L5_ROOT` for every Agent.
