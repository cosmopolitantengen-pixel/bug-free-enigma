...
`POST /skills/runs/request` is the controlled execution entrypoint. It validates the Skill input schema, enabled state, two-sided Agent authorization, permission boundary, registered risk, and approval requirement before dispatching a deterministic adapter. Every request creates a durable `SkillRun`; successful runs also write an Evaluation, blocked runs create an Incident, and approval-gated runs can continue only through `POST /skills/runs/{run_id}/complete` after a Human Root decision.

The local V1 runtime has adapters for all 18 bootstrap Skills. State-writing behavior is deliberately narrow: Memory Write may append scoped task memory, while approval/audit Skills prepare structured records rather than bypassing their owning control services. Code generation remains draft-only and does not execute code.
