...
Live providers are opt-in. Generation supports a governed local Codex CLI core, native OpenAI Responses, and DeepSeek Chat Completions, with per-request provider/model selection and explicit fallback. On a machine already authenticated with `codex login`, use `AI_COMPANY_OS_ENABLE_CODEX_CLI=true` with `AI_COMPANY_OS_MODEL_PROVIDER=codex`. Codex runs ephemerally in a read-only sandbox; mutations still pass through AI Company OS approvals and tools. Set `DEEPSEEK_API_KEY` for `deepseek`, or `OPENAI_API_KEY` for `openai`. OpenAI remains the embedding provider for 1536-dimensional pgvector Knowledge indexing.
